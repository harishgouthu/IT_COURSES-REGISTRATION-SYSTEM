...
// const AdminPasswordResetRequest = ({ showModal, handleCloseModal, modalRef }) => {
...
//         try {
//             await axios.post('http://127.0.0.1:8000/users/Adminpassword-reset/', { email });
...
//                             &times;
//                         </button>
//                         <h2 className="text-xl font-semibold mb-4 text-blue-600 text-center">AdminPassword Reset Request</h2>
...
//                             >
//                                 Send AdminPassword Reset Email
//                             </button>
//                         </form>
...
const AdminPasswordResetRequest = ({ showModal, handleCloseModal, modalRef }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setError('');
            setEmail(''); // Clear the email input
            // Delay closing modal to let user see the message
            setTimeout(() => {
                handleCloseModal(); // Close modal after 3 seconds
            }, 3000);
        } catch (err) {
            setError(err.response?.data?.error || 'An error occurred.');
            setMessage('');
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-60">
                    ...
                    >
                        ...
                        <h2 className="text-xl font-semibold mb-4 text-blue-600 text-center">Admin Password Reset Request</h2>
                        <form onSubmit={handleSubmit}>
                            <label className="block mb-2 text-sm font-medium text-gray-600">Email</label>
                            <input
                                type="email"
